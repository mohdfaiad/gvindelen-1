...
uses
  Math, DAConsts, CRParser, CRFunctions,
...
procedure TCustomIBCDataSetService.FillFieldsDefaultValues;
  ...
  begin
    ...
    if DefaultExpressionOldBehavior then begin
      ...
      if (Field is TNumericField) and ({$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator <> '.') and
         (Pos('.', DefExpr) > 0) then
         DefExpr[Pos('.', DefExpr)] := _char({$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator{$IFDEF CLR}[1]{$ENDIF});
      if DefExpr = '' then
...
begin
  ...
  try
    if Parser.ToLexem(lxSELECT, lxWITH, True) <> lcEnd then begin
        ...
        if Con.IsFBServer and (Con.GetMajorServerVersion >= 2) then begin
          ...
          if HaveUnion then
            ...
            s := 'SELECT COUNT(*) FROM (' + LineSeparator +
              ...
              Copy(s, FromPos - 4 {length('FROM')}, DelimiterPos - FromPos + 4) + LineSeparator + ') q' +
              Copy(s, DelimiterPos, MaxInt);
        end
        else
