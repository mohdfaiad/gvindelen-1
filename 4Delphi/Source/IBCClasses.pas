...
{$ENDIF}
{$ENDIF}
  SysUtils, Classes, SyncObjs, MemData, MemUtils, CRTypes, CRParser, CRAccess,
{$IFNDEF UNIDACPRO}
  IBCCall, IBCError;
{$ELSE}
...
begin
  if not FConnected then begin
    ...
    try
      try
        GDS.Busy;
        Res := GDS.isc_attach_database(FStatusVector, Length(FullDBName), pName, FDatabaseHandle, FDPBLength, {$IFDEF FPC}PChar{$ENDIF}(FDPB));
        GDS.Release;
        Check(Res);
      except
    ...
    case FDataType of
      ...
      dtDateTime, dtDate, dtTime:
        ...
        {$IFNDEF FPC}
          ...
          if VarIsSQLTimeStamp(Value) then
            SetAsDateTime(SqlTimSt.SQLTimeStampToDateTime(VarToSQLTimeStamp(Value)))
        {$ENDIF}
        {$ENDIF}
          else
            raise EConvertError.Create(SCannotConvertType);
...
procedure TGDSRecordSet.InternalInitFields;
  ...
  begin
    ...
    try
      if Parser.ToLexem(lxSELECT, lxWITH, True) <> lcEnd then
...
begin
  ...
  {$ELSE}
    for i := 1 to Scale - StrLen + 2 do
      bcdstr[i] := '0';
    bcdstr[2] := {$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator;
