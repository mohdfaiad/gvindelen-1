...
{$ELSE}
  Types,
{$ENDIF}
  SysUtils, Classes, Math, CRTypes, MemUtils, DAConsts,
...
{ GDS functions definitions }
  ...
  Tisc_attach_database = function (status_vector: PISC_STATUS; db_name_length: Short;
                             ...
                             parm_buffer_length: Short; parm_buffer: {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF}): ISC_STATUS;
                             {$IFNDEF CLR}
  ...
  Tisc_create_blob2 = function (status_vector : PISC_STATUS; db_handle : PISC_DB_HANDLE;
                          ...
                          blob_id : PISC_QUAD; bpb_length : Short; bpb_address : {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF}): ISC_STATUS;
                          {$IFNDEF CLR}
                          {$IFDEF MSWINDOWS}stdcall;{$ELSE}cdecl;{$ENDIF}
                          {$ENDIF}
  ...
  Tisc_open_blob2 = function (status_vector: PISC_STATUS; db_handle: PISC_DB_HANDLE;
                        tran_handle: PISC_TR_HANDLE; blob_handle: PISC_BLOB_HANDLE;
                        blob_id: PISC_QUAD; bpb_length: Short; bpb_buffer: {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF}): ISC_STATUS;
                        {$IFNDEF CLR}
                        {$IFDEF MSWINDOWS}stdcall;{$ELSE}cdecl;{$ENDIF}
                        {$ENDIF}
...
{ Other Blob functions }
  ...
  Tisc_service_attach = function (status_vector: PISC_STATUS;
                                  ...
                                  isc_arg5: UShort;
                                  isc_arg6: {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF}): ISC_STATUS;
                                  {$IFNDEF CLR}
                                  {$IFDEF MSWINDOWS}stdcall;{$ELSE}cdecl;{$ENDIF}
                                  {$ENDIF}
  ...
  Tisc_service_query = function (status_vector: PISC_STATUS;
                                 ...
                                 isc_arg4: UShort;
                                 isc_arg5: {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF};
                                 isc_arg6: UShort;
                                 isc_arg7: {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF};
                                 isc_arg8: UShort;
  ...
  Tisc_service_start = function (status_vector: PISC_STATUS;
                                 ...
                                 recv_handle: PISC_SVC_HANDLE;
                                 isc_arg4: UShort;
                                 isc_arg5: {$IFDEF FPC}PChar{$ELSE}TBytes{$ENDIF}): ISC_STATUS;
                                 {$IFNDEF CLR}
                                 {$IFDEF MSWINDOWS}stdcall;{$ELSE}cdecl;{$ENDIF}
                                 {$ENDIF}
...
{$ELSE}
{$IFDEF MACOS}
  GDSDLLName = 'libibtogo.dylib';
  FBClientDLLName = 'libfbclient.dylib';
  IBXMLDLLName = 'libibxml.dylib';
{$ELSE}
...
{$IFDEF POSIX}
  if GDSDLL = '' then begin
    GDSDLL := GDSDLLName;
    hIBCLib := dlopen(PAnsiChar(AnsiString(GDSDLLName)), {RTLD_GLOBAL}RTLD_LAZY);
    if hIBCLib = 0 then begin
      hIBCLib := dlopen(PAnsiChar(AnsiString(FBClientDLLName)), {RTLD_GLOBAL}RTLD_LAZY);
      if hIBCLib <> 0 then
        GDSDLL := FBClientDLLName;
    end;
    NeedLoadGDS := False;
...
{$ENDIF}
  ...
  {$ELSE}
    hIBCLib := dlopen(PAnsiChar(AnsiString(GDSDLL)), {RTLD_GLOBAL}RTLD_LAZY);
  {$ENDIF}
...
{$ENDIF}
{$IFDEF MACOS}
  if Pos('libibtogo.dylib', GDSDLL) <= 0 then
    hXMLLib := 0
  else
    hXMLLib := dlopen(PAnsiChar(AnsiString(IBXMLDLL)), {RTLD_GLOBAL}RTLD_LAZY);
{$ENDIF}
...
{$ELSE}
  Result := dlsym(hIBCLib, PAnsiChar(AnsiString(Name)));
{$ENDIF}
  if Result = nil then
    Result := @NotLink;
end;
...
{$ELSE}
  Result := dlsym(hXMLLib, PAnsiChar(AnsiString(Name)));
{$ENDIF}
  if Result = nil then
    Result := @NotLink;
end;
...
{$ELSE}
  ...
  if @isc_get_client_version <> @NotLink then begin
    ...
    if p1 > 0 then begin
      ...
      Version := StrToFloat(StringReplace(VersionSt, '.', {$IFDEF VER16P}FormatSettings.{$ENDIF}DecimalSeparator, []));
    end
    else begin
      VersionSt := IntToStr(isc_get_client_major_version) + '.' + IntToStr(isc_get_client_minor_version);
      Version := isc_get_client_major_version + (isc_get_client_minor_version / 10);
