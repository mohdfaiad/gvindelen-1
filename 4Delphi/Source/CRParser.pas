...
{$ENDIF}
  ...
  public
    ...
    function ToLexem(const Code: integer; const SkipLexem: integer; const SkipSubQueries: boolean): integer; overload;
...
function TParser.ToLexem(const Code: integer; const SkipLexem: integer; const SkipSubQueries: boolean): integer;
var
  St: _string;
begin
  repeat
    Result := GetNext(St);
  until ((Result <> lcBlank) and (Result <> lcComment)) or (Result = lcEnd);

  if Result = SkipLexem then
    Result := ToLexem(Code, SkipSubQueries)
  else
  if (Result <> Code) and (Result <> lcEnd) then
    Result := ToLexem(Code, False);
end;
