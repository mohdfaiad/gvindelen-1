...
type
  ...
  protected
    ...
    function InternalPopup(X, Y: Integer; ForceFocus: Boolean; PopupControl: TControl = nil): Boolean; override;
  public
...
function TSpTBXColorEditPopupMenu.InternalPopup(X, Y: Integer;
  ForceFocus: Boolean; PopupControl: TControl): Boolean;
begin
  if Assigned(PopupControl) and (PopupControl is TSpTBXColorEdit) then
    SkinType := TSpTBXColorEdit(PopupControl).SkinType;
  Result := inherited InternalPopup(X, Y, ForceFocus, PopupControl);
end;
...
begin
  if Assigned(ActiveFormPopupMenu) then begin
    T := ActiveFormPopupMenu.SkinType;
    SkinManager.ChangeControlSkinType(Self, T);
    if T <> sknSkin then
...
begin
  if (PaintStage = pstPrePaint) and (Sender is TSpTBXSpeedButton) then begin
    ...
    if B.MouseInControl then begin
      case SpTBXSkinType(SpTBXPanel1.SkinType) of
        sknNone:
          PaintDefault := True;
        sknWindows:
          begin
...
begin
  if PaintStage = pstPrePaint then begin
    PaintDefault := False;
    SpDrawXPDock(ACanvas, ARect, SpTBXPanel1.SkinType);
    SpDrawXPToolbar(ACanvas, ARect, SpTBXPanel1.SkinType, True, False, False, True, False);
  end;
end;
