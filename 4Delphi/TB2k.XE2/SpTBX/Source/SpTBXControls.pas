...
begin
  ...
  if (not DoubleBuffered or (Message.wParam = WPARAM(TMessage(Message).lParam))) and
...
begin
  if not DoubleBuffered or (Message.wParam = WPARAM(TMessage(Message).lParam)) then
  begin
        ...
        begin
          ...
          case Message of
            ...
            WM_MBUTTONDOWN, WM_MBUTTONDBLCLK: begin
                {$IF CompilerVersion >= 23} // for Delphi XE2 and up
                P := SmallPointToPoint(Types.SmallPoint(Integer(lParam)));
                {$ELSE}
...
begin
  if SpTBXSkinType(SkinType) = sknWindows then
