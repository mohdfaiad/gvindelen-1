      ...
      sknWindows:
        if ItemInfo.IsDesigning then
          ...
          if ItemInfo.IsOnMenuBar then begin
            if SpIsWinVistaOrUp or SkinManager.IsCustomStyleActive then begin
  ...
  begin
    case ItemInfo.SkinType of
      sknNone:
        SpFillRect(ACanvas, ARect, clHighlight);
      sknWindows:
        if SpIsWinVistaOrUp or SkinManager.IsCustomStyleActive then
...
begin
  ...
  if (SkinType = sknWindows) and not SpIsWinVistaOrUp and not SkinManager.IsCustomStyleActive then
    SkinType := sknNone;

  case SkinType of
    sknNone:; // No gutter on Windows 9x, 2000 and XP
    sknWindows: // Only Windows Vista painting
  ...
  case SkinType of
    ...
    sknWindows:
      if PaintBorders and Docked then  // Paint only the borders
        if SkinManager.IsCustomStyleActive then begin  // XE2 Styles
...
var
  ...
  Details: TThemedElementDetails;
begin
  if SpIsWinVistaOrUp and SkinManager.IsXPThemesEnabled and not SkinManager.IsCustomStyleActive then begin
...
begin
  ...
  if not DoubleBuffered or (Message.wParam = WPARAM(TMessage(Message).lParam)) then begin
    B := TBitmap.Create;
    try
      ARect := GetClientRect;
      B.Width := ARect.Right;
