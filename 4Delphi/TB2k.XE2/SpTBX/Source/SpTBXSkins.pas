...
type
  ...
  public
    ...
    function GetThemedElementTextColor(Details: TThemedElementDetails): TColor;
...
begin
  ...
  ACanvas.Font.Color := CurrentSkin.GetTextColor(skncListItem, State, SkinType);
  ...
  if SpTBXSkinType(SkinType) = sknSkin then begin
    ...
    if SkinManager.IsCustomStyleActive then begin
      {$IF CompilerVersion >= 23} // for Delphi XE2 and up
      if Selected then
...
begin
  ...
  case Component of
    skncMenuItem:
      if (SkinType = sknWindows) and (SpIsWinVistaOrUp or SkinManager.IsCustomStyleActive) then begin
        // Use the new API on Windows Vista
        if CurrentSkin.GetThemedElementDetails(Component, State, Details) then
          Result := CurrentSkin.GetThemedElementTextColor(Details);
      end
      else
    ...
    skncToolbarItem:
      if (SkinType = sknWindows) and SkinManager.IsCustomStyleActive then begin
        if CurrentSkin.GetThemedElementDetails(Component, State, Details) then
          Result := CurrentSkin.GetThemedElementTextColor(Details);
      end
      else
    ...
    skncTabToolbar:
      if SkinType = sknSkin then
        ...
        if SkinType = sknWindows then
          Result := GetTextColor(skncTab, State, SkinType);
    skncWindowTitleBar:
      if SkinType = sknSkin then
        Result := GetTextColor(skncToolbarItem, State, SkinType)  // Use skncToolbarItem to get the default text color
      else
...
begin
  ...
  case Component of
    skncDock:
      begin
        if SkinManager.IsCustomStyleActive then
...
function TSpTBXSkinOptions.GetThemedElementTextColor(Details: TThemedElementDetails): TColor;
begin
  {$IF CompilerVersion >= 23} // for Delphi XE2 and up
  SpTBXThemeServices.GetElementColor(Details, ecTextColor, Result);
  {$ELSE}
...
var
  ...
  Details: TThemedElementDetails;
begin
  if MenuItemStyle and (SpIsWinVistaOrUp or SkinManager.IsCustomStyleActive) and (SkinManager.GetSkinType = sknWindows) then begin
    // Use the new API on Windows Vista
    Details.Element := teMenu;
...
function TSpTBXSkinManager.IsCustomStyleActive: Boolean;
begin
  {$IF CompilerVersion >= 23} // for Delphi XE2 and up
