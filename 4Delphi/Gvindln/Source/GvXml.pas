...
{$ENDIF}
  ...
  private
    ...
    procedure SetAttrOrChildValue(const Name: String; const Value: Variant);
  protected
...
begin
  try
    result:= FAttributes.Find(AttrName).AsString;
  except
    on E: EAttrNotExists do
      Result := '';
  end;
...
function TGvXmlNode.GetValue: Variant;
begin
  Result:= FValue;
end;
...
procedure TGvXmlNode.SetAttrOrChildValue(const Name: String;
  const Value: Variant);
var
  Child: TGvXmlNode;
begin
...
procedure TGvXmlNode.SetValue(const Value: Variant);
begin
  FValue:= Value;
end;
...
{$ENDIF}
  ...
  public
    ...
    constructor Create(aNodeName: string); overload;
    constructor Create; overload; virtual;
    constructor Clone(aNode: TGvXmlNode);
    destructor Destroy; override;
    procedure Clear;
    function Find(aNodeName: String): TGvXmlNode; overload;
    function Find(aNodeName, aAttrName: String): TGvXmlNode; overload;
...
constructor TGvXmlNode.Clone(aNode: TGvXmlNode);
begin
  inherited Create;
  Assign(aNode);
end;
