...
{begin
  ...
  case Field.DataType of
    ftInteger:
      begin
        Move(Buffer^, vInteger, sizeof(Integer));
        Node[Field.FieldName]:= vInteger;
      end;
    ftString:
      begin
        SetLength(vString, StrLen(PAnsiChar(Buffer)));
        StrCopy(PAnsiChar(vString), PAnsiChar(Buffer));
        Node.Attr[Field.FieldName].AsString:= vString;
      end;
    ftDateTime:
      begin
        Move(Buffer^, vDateTime, sizeof (TDateTime));
        Node[Field.FieldName]:= vDateTime;
      end;
    ftBoolean:
      begin
        Move(Buffer^, vBool, sizeof (WordBool));
        Node[Field.FieldName]:= vBool;
      end;
  else
    raise EGvDataSetError.Create('Unsupported DataType');
  end;
  SetModified(True);
end;}
