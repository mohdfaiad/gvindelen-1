      ...
      begin
        ...
        try
          case ASelectionType of
            gstNon:
              begin
                if not ( (DBGridEh.DataSource <> nil) and (DBGridEh.DataSource.DataSet <> nil) and
                      not DBGridEh.DataSource.DataSet.IsEmpty )
                then
                  Exit;
                ColList := TColumnsEhList.Create;
                IsFreeColList := True;
...
begin
  if Column.Checkboxes then
    ...
    FVarValues[FCurRow-1-FTitleRows, FCurCol-1] := Column.Field.Value
  else
