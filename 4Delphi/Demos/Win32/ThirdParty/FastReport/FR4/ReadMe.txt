...
Demo for FastReport included in IBDAC was built and tested using
Fast Report 4
...
Before using Demo you should install FastReport 4 IBDAC Components.
The following instruction will help you to compile and install 
FastReport 4 IBDAC Components manually using IDE.

C++Builder 2007
---------------
If you have only C++Builder 2007, you should perform the following steps:

  1. Make sure that you have IBDAC and FastReport installed
  ...
  7. Run IDE and add dclfrxIBDAC11.bpl and dclfsIBDAC11.bpl via Component->Install Packages... menu 
  8. To compile applications with IBDAC for FastReport components, add PROJECTPATH to the "Library Path" and "Include Path" lists
...
Run your IDE and walk through the following steps:
  ...
  2) Compile IBDAC run-time package (frxIBDACXX.dpk)
  3) Compile and install IBDAC design-time package (dclfrxIBDACXX.dpk)
...
To compile applications based on FastReport 4 IBDAC Components, add the following 
path to the "Library Path":
%IBDAC%\Demos\Win32\ThirdParty\FastReport\FR4\
where %IBDAC% is the IBDAC installation path on your computer.