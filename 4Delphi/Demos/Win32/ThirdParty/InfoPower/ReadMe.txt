Interbase Data Access Components
Copyright 1997-2011, Devart. All Rights Reserved
--------------------------------------------------

This demo was built and tested using 
InfoPower 2000 Trial for Delphi 5 and InfoPower 4000 Trial for Delphi 7

To install TwwIBCQuery, TwwIBCTable, and TwwIBCStoredProc components, 
compile and install the IBCIPPack package.
The IBCIPPack package requires InfoPower (ip*.dcp). To make it, you need
to compile the InfoPower package (ip*.dpk).

You can recompile it under other versions by changing the Requires section
in the IBCIPPack.dpk file manually.

IMPORTANT NOTE:
  This demo is provided "as is", and there is no warranty that it is fully
  compatible with other versions of InfoPower.