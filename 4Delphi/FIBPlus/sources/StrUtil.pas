...
begin
  ...
  Move((PChar(Integer(FMemory) + Integer(FPosition)))^, Buffer, Result);
...
procedure TFastStringStream.SetSize(NewSize: Integer);
begin
  ...
  begin
    ...
    if FActualLength>NewSize then FActualLength := NewSize;
  end;
end;
...
begin
  ...
  begin
    if FPosition + Result > FSize then
     Grow(Result);
    Move(Buffer, (PChar(Integer(FMemory) + Integer(FPosition)*FElementSize))^,
     Result*SizeOf(Char)
    );
    Inc(FPosition, Result);
