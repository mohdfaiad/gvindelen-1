...
begin
  ...
  FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockCount-1]:=AllocMem(FBlockSize);
  Result:=BlockCount;
end;
...
begin
 ...
 if BlockNo=BlockCount then
  CreateNewBlock;
 Result:=dbPChar(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockNo])+RecordPosition(IRecordNo,True);
end;
...
begin
  ...
  Result :=dbPChar(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockIndex])+PosInBlock;
end;
...
begin
  ...
  if Old  then
   Cache:=dbPChar(FChangeLog.List{$IFNDEF D16+}^{$ENDIF}[BlockIndex])+PosInBlock
  else
   Cache:=dbPChar(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockIndex])+PosInBlock;
  if (FStrFieldCount=0) then
...
begin
 if Assigned(FMapRecords) then
  if RecordNo<=FMapRecords.Count then
   Result :=Integer(FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[RecordNo-1])
  else
   Result := -1
 else
  Result :=RecordNo
...
begin
  ...
  begin
    PosInBlock:=RecordPosition(RecordNo);
    Cache:=dbPChar(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockIndex])+PosInBlock+FieldOffSet;
    if Data=nil then
...
// Используется в Locate
  ...
  begin
      ...
      RecordData:=FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockIndex];
...
var
   EndBlock:pointer;
begin
  EndBlock:=FBlocks.List{$IFNDEF D16+}^{$ENDIF}[CurBlock];
  ...
  begin
   Inc(CurBlock);
   Result:=FBlocks.List{$IFNDEF D16+}^{$ENDIF}[CurBlock]
  end
end;
...
// if ((p^.InternalNo mod FRecInBlock)=1) and (BlockIndex>=ChangesBlockCount) then
 ...
 begin
   ...
   FChangeLog.List{$IFNDEF D16+}^{$ENDIF}[ChangesBlockCount-1]:=AllocMem(FBlockSize);
   if Assigned(FStringData) and not Assigned(FLogStringData) then
      FLogStringData    :=TStringCollection.Create(FStrFieldCount);
 end;


 Move(dbPChar(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[BlockIndex])[PosInBlock],dbPChar(FChangeLog.List{$IFNDEF D16+}^{$ENDIF}[ChangeBlockIndex])[ChangePosInBlock],FRecordSize);
 if Assigned(FStringData) then
 begin
...
begin
 InitializeMap;
 c:=Integer(FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[OldRecordNo]);
 FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[OldRecordNo]:=FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[NewRecordNo];
 FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[NewRecordNo]:=Pointer(c);
end;
...
begin
 ...
 if NewRecno>OldRecno then
  Inc(NewRecno);
 InitializeMap;
 Position:=Integer(FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[OldRecNo]);
...
begin
  ...
  begin
   ...
   for i := 0 to BlockCount - 1 do
    WriteBuffer(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[i]^,FBlockSize);
...
begin
  for i := 0 to BlockCount - 1 do
  begin
    FreeMem(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[i]);
  end;
  FBlocks.Clear;
  FMapRecords.Free;
  FMapRecords:=nil;
  ...
  begin
   ...
   begin
    GetMem(p,FBlockSize);
    FBlocks.List{$IFNDEF D16+}^{$ENDIF}[i]:=p;
    ReadBuffer(FBlocks.List{$IFNDEF D16+}^{$ENDIF}[i]^,FBlockSize);
   end;
   if FStrFieldCount>0 then
   begin
    FStringData    :=TStringCollection.Create(FStrFieldCount);
...
begin
  ...
  begin
    ...
    for i := 0 to FMapRecords.Count - 1  do
    begin
      FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[i]:=Pointer(i+1);
    end;
  end;
end;
...
begin
  ...
  begin
    for i := 0 to FMapRecords.Count - 1  do
    begin
      FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[i]:=Pointer(Integer(FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[i])+Distance);
    end;
  end;
end;
...
begin
 if Assigned(FMapRecords) then
 begin
   if Integer(FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[RecordNo-1])=FRecordCount then
    Dec(FRecordCount);
   if Assigned(FStringData) then
    FStringData.Delete(Integer(FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[RecordNo-1])-1);
...
begin
 if Assigned(FMapRecords) then
 begin
  if (BookMark<=FMapRecords.Count) and (FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[BookMark-1]=Pointer(BookMark)) then
   Result:=BookMark-1
  else
...
begin
 ...
 begin
   if Assigned(FMapRecords) then
   begin
    Result:=(BookMark<FMapRecords.Count) and (FMapRecords.List{$IFNDEF D16+}^{$ENDIF}[BookMark-1]=Pointer(BookMark));
    if not Result then
