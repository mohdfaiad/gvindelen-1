...
{$ENDIF}
  ...
  published
    property MDTTransactionRole: TFIBMDTTransactionRole
      read FMDTTransactionRole write SetMDTTransactionRole;
  end;
...
begin
  raise Exception.Create(AMethodName+ '- не реализовано для MDT режима');
  end;
    ...
    try
      if FMDTTransactionRole=mtrSeparateTransaction then begin
        if FMDTIBRealDBTransaction.Active then begin
          case Action of
            TACommit:begin
              MDTDatabase.CommitSepareteTransaction(FMDTIBRealDBTransaction);
              FMDTIBRealDBTransaction.GetOwnerObject.Free;
              FMDTIBRealDBTransaction:=nil;
              end;
            TARollback:begin
            ...
            TACommitRetaining:
              FMDTIBRealDBTransaction.CommitRetaining;
            TARollbackRetaining:
        ...
        XUsedLongTransaction:=
          FMDTIBRealDBTransaction=MDTIBRealDBCanal.IMDTIBRealLongTransaction;
        if (not XUsedLongTransaction) or (Action in [TACommit,TARollback])
          then FMDTIBRealDBTransaction:=nil;
//        FDeclaredSavePoints.Clear;
        case Action of
          TACommit:
            if not XUsedLongTransaction then
              while MDTDatabase.ProtectedBlocks.BlockCount>0
                do MDTDatabase.CommitProtectedBlock;
          TARollback:
            if not XUsedLongTransaction then
              while MDTDatabase.ProtectedBlocks.BlockCount>0
                do MDTDatabase.RollbackProtectedBlock;
          TACommitRetaining:
            if not XUsedLongTransaction
              then MDTNotSupport('TFIBTransaction.EndTransaction TACommitRetaining');
          TARollbackRetaining:
            if not XUsedLongTransaction
              then MDTNotSupport('TFIBTransaction.EndTransaction TACommitRetaining');
          end;
...
begin
  try
    ...
    for i := 0 to DatabaseCount - 1 do  begin
      Databases[i].CheckActive(not (trsInLoaded in FTransactionRunStates));
      if not Databases[i].Connected then exit;
      end;
    (*
    ...
    if Assigned(XIBTransactionParams)
      then XIBTransactionParams.GetOwnerObject.Free;
    end;
end;
...
begin
  Result:=Assigned(MDTDatabase) and
     not (FMDTTransactionRole in [mtrNone,mtrSeparateTransaction]);
  end;
