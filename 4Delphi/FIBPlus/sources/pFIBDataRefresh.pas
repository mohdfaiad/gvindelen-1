...
type
  ...
  // For one Table only
     ...
     private
//        Flags
      FCommitOrder:Int64; // обработанный коммит
      FCurTransaction:Int64;                // Для вычитки изменений в тек транзакции и обработки RollBack
...
begin
 FCommitOrder:=0;
 FCurTransaction:=0;   // Для вычитки изменений в тек транзакции и обработки RollBack
 FLastTimeReaded:=0;
end;
...
begin
 FLastTimeReaded:=0;
 FCurTransaction:=0; // Номер транзакции обнулился в триггерах
...
begin
 ...
 begin
  FLastTimeReaded:=0;
  ReceiveDelta(True) // Изменения которые щас будут отменены.
 end;
...
var
   ...
   tmpTime:TDateTime;
begin
   SetCurTransactionID; // заодно транзакция прочекилась
   PrepareSQLTexts;
