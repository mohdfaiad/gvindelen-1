 ...
 try
   ...
   begin
    ...
    for i:=0 to fl.Count-1 do
     SrcKeys[i]:=RefreshQuery.FieldByName(TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName);
...
begin
 ...
 try
   ...
   SetLength(SrcKeys,fl.Count);
   for i:=0 to fl.Count-1 do
     SrcKeys[i]:=RefreshDataSet.FieldByName(TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName);
   SetLength(KeyValues,fl.Count);
