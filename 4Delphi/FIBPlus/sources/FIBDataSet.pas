...
{$ENDIF}
 ...
 {$ENDIF}
  ...
  TDataSetRunStateValue=(drsInCacheRefresh,drsInSort,drsInOpenByTimer,
   ...
   drsGetBlobStream,drsInLoadFromStream
  );
  ...
  if XMDTDataRecord<>nil then begin
     ...
     for i := 0 to c do begin
       ...
       if curVar.IsBlob then begin
         XField:=Fields[I+1];
         XMDTDataRecord.GetDataBuffer(I,XBlobDataBufer,XBlobDataSize);
         XStr:=XMDTDataRecord.StringValue[I];
...
begin
  ...
  try
    ...
    begin
      fn:=
       FieldNameForSQL(TableAliasForField(TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName),
        GetRelationFieldName(TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i])));
      ...
      case VarType(KeyValues[i]) of
       ...
       begin
        ...
        begin
         ParName:='UPPER(?'+FormatIdentifier(3,LocateParamPrefix+TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName)+')';
         fn:='UPPER('+fn+')'
        end
        else
         ParName:='?'+FormatIdentifier(3,LocateParamPrefix+TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName);
       ...
       vLocateWhere:=
        vLocateWhere+'('+fn+'=?'+FormatIdentifier(3,LocateParamPrefix+TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName)+')';
      end
    end;
    ...
    begin
      if Length(KeyValues)>i then
       vQLocate.Params.ByName[LocateParamPrefix+TField(fl.List{$IFNDEF D16+}^{$ENDIF}[i]).FieldName].Value:=KeyValues[i]
    end;
...
{$IFDEF D16+}
procedure TFIBCustomDataSet.DataEvent(Event: TDataEvent; Info: NativeInt);
{$ELSE}
