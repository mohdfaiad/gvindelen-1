...
//  Array Support
 ...
 {$ENDIF}
  ...
  public
    ...
    procedure MDTInitByVariables(AVariables: IMDTVariables);
...
//       vNeedUTFEncode:= sSubType=1; // Будем считать что все проверено
       FIsDefferedSetting:=False;
      end;
     else
...
procedure TFIBXSQLDA.MDTInitByVariables(AVariables:IMDTVariables);
var
  I:integer;
...
begin
  ...
  for I:=0 to FCount-1 do begin
    XMDTVariable:=AVariables.IVariable[I];
    ...
    case XMDTVariable.StructureType of
      mdt_vstString:begin
        ...
        if XMDTVariable.AlwaysMaxLength then begin
          ...
          XXSQLVAR^.sqllen:=XMDTVariable.MaxLength;
          end
        else begin
          XXSQLVAR^.sqltype:=SQL_VARYING;
          XXSQLVAR^.sqllen:=XMDTVariable.MaxLength+2;
          end;
        end;
      ...
      mdt_vstExtended:begin
        if XMDTVariable.Precision=0 then begin //???Точность пока правильно не определяется для SQL выражений,
          XXSQLVAR^.sqltype:=SQL_DOUBLE;
          XXSQLVAR^.sqllen:=SizeOf(Double);
          XXSQLVAR^.sqlscale:=0;
          end
...
var
  ...
  procedure XTypesError; {MDT ???Причины,имена}
  begin
...
begin
  for I:=0 to FCount-1 do begin
    XMDTVariable:=ADataRecord.IVariables.IVariable[I];
    XFIBXSQLVAR:=FXSQLVARs^[i];
    XXSQLVAR:=XFIBXSQLVAR.Data;
    if ADataRecord.IsNull[I] then begin
      ...
      ADataRecord.GetDataBuffer(I,XDataBuffer,XSize);
      case XMDTVariable.StructureType of
        mdt_vstString:begin
...
begin
  for I:=0 to FCount-1 do begin
    ...
    XFIBXSQLVAR:=FXSQLVARs^[i];
    XXSQLVAR:=XFIBXSQLVAR.Data;
    if XFIBXSQLVAR.IsNull then ADataRecord.ClearValue(I)
    else begin
      XSQLData:=XXSQLVAR^.sqldata;
      if XSQLData=nil then XTypesError;
      case XMDTVariable.StructureType of
        mdt_vstString:begin
          case XFIBXSQLVAR.FSrvSQLType of
            SQL_VARYING:begin
              ...
              Inc(Integer(XSQLData),2);
              ADataRecord.WriteDataToBuffer(I,XSQLData,XSize);
              end;
            SQL_TEXT:begin
            ...
            SQL_BLOB:begin
              if XFIBXSQLVAR.FStreamValue=nil
                then XTypesError;
              ADataRecord.WriteDataToBuffer(I,XFIBXSQLVAR.FStreamValue);
              end;
            else XTypesError;
            end;
          end;
        mdt_vstInteger:begin
          if (XFIBXSQLVAR.FSrvSQLType<>SQL_LONG) and
             (XFIBXSQLVAR.Scale<>0) then XTypesError;
          ADataRecord.IntegerValue[I]:=PLong(XSQLData)^;
          end;
        mdt_vstSmallInt:begin
          if (XFIBXSQLVAR.FSrvSQLType<>SQL_SHORT) and
             (XFIBXSQLVAR.Scale<>0) then XTypesError;
          ADataRecord.SmallIntValue[I]:=PShort(XSQLData)^;
          end;
        mdt_vstInt64:begin
          if (XFIBXSQLVAR.FSrvSQLType<>SQL_INT64) and
             (XFIBXSQLVAR.Scale<>0) then XTypesError;
          ADataRecord.Int64Value[I]:=PInt64(XSQLData)^
          end;
        mdt_vstDouble,mdt_vstExtended:begin
          XExtended:=0;
          case XFIBXSQLVAR.FSrvSQLType of
          ...
          if XMDTVariable.StructureType=mdt_vstDouble
            then ADataRecord.DoubleValue[I]:=XExtended
            else ADataRecord.ExtendedValue[I]:=XExtended;
          end;
        mdt_vstTimeStamp:begin
          case XFIBXSQLVAR.FSrvSQLType of
            SQL_TIMESTAMP:begin
          ...
          ADataRecord.TimeStampValue[I]:=XTimeStamp;
          end;
        else XTypesError;
  {
...
begin
  ...
  if Assigned(FMDTMainDataOrder) then begin
    ...
    then Result:=FMDTFindTableByField.FindTableByField(FieldIndex)
    else Result:='';
  end
  else
  begin
  ...
  begin
    ...
    try
      try
        ...
        FSQLRecord.MDTInitByVariables(FMDTResultDataRecord.IVariables);
      ...
      XSelectParser.GetOwnerObject.Free;
      XMDTSQLBuildDataOrderAddData.GetOwnerObject.Free;
      if Assigned(XSelect)
        then XSelect.GetOwnerObject.Free;
      end;
    end;

  function XDoForMDT:boolean;
  begin
    if (not Transaction.IsUsingMDT) or  (FMDTSQLExecutor=se_Server) then begin
      Result:=false;
      exit;
      end
    else Result:=true;
...
begin
  if Assigned(Database)
    then result:=Database.MDTDatabase
    else result:=nil;
  end;
