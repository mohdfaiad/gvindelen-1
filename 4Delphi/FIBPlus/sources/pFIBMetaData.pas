...
{ TODO : Descriptions }
  ...
  TDDLTextOption=(dtoUseCreateDB,dtoUseSetTerm,dtoDecodeDomains,dtoIncludeGrants);
...
begin
  if (Length(FMessage)=0) or (FMessage[1]<>'''') then
   Stream.WriteString(Format('CREATE EXCEPTION %s ''%s''', [FormatName, FMessage]))
  else
   Stream.WriteString(Format('CREATE EXCEPTION %s ''''%s''''', [FormatName, FMessage]))
end;
