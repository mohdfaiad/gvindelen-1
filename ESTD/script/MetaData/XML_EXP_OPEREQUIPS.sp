CREATE OR ALTER PROCEDURE XML_EXP_OPEREQUIPS (
...
begin
  for select operequip_id, equip_id
   from operequips
  ...
  begin
    ...
    execute procedure xml_create_elem
      :nd_oper, 'EquipmentList/Equipment[]', :v_obj_label
      returning_values :nd_operobj, :v_is_new;
    for select oda.attr_code, oda.kei, oda.attr_value
      from operequipattrs oda
      where oda.operequip_id = :v_operobj_id
