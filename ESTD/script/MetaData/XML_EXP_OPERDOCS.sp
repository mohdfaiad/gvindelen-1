CREATE OR ALTER PROCEDURE XML_EXP_OPERDOCS (
...
begin
  for select operdoc_id, document_id
   from operdocs
   where oper_id = :i_oper_id
   into :v_operobj_id, :v_obj_id do
  begin
    execute procedure xml_exp_doc(:nd_root, :v_obj_id) returning_values :v_obj_label;
    execute procedure xml_create_elem
      :nd_oper, 'DocumentList/Document[]', :v_obj_label
      returning_values :nd_operobj, :v_is_new;
    for select oda.attr_code, oda.kei, oda.attr_value
      from operdocattrs oda
      where oda.operdoc_id = :v_operobj_id
