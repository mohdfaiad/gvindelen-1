...
/*    if($('#isa_navigation li.active').length) {
		...
		var activeA = $('<a style="position:absolute;" href="' + originalA.attr('href') + '"><img class="' + activeLi.attr('id') + '" src="' + originalImg.attr('src').replace(/\.gif/g, '_aktiv.gif') + '" /></a>');
		activeA.css({
			left: activeLi.offset().left - 10 + 'px',
  ...
  */
    ...
    $('#isa_navigation li.link').hover(function () {
        ...
        var li = $(this);
        var img = li.find('img');
        var markedImglink = $('<a style="position:absolute;" class="' + li.attr('id') + '" href="' + img.parent('a').attr('href') + '"><img class="' + li.attr('id') + '" src="' + img.attr('src').replace(/\.gif/g, '_over.gif') + '" /></a>');
        markedImglink.mouseout(function () {
            openEntryTimer = window.setTimeout(closeOpenEntry, 300);
        });
        markedImglink.css({
            left: li.offset().left - 10 + 'px',
...
//        div.css('background-color', div.children('table').bgcolor);
        div.css({
