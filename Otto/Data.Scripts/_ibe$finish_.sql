...
begin
  ...
  if (new.magazine_name is null) then
    select trim(c.catalog_name)||' äî '||new.valid_date
      from catalogs c
...
begin
  ...
  begin
    ...
    begin
      ...
      v_error = v_error_message||' (îáúåêò="'||:i_object_sign||'" id="'||:i_object_id||'" ïåðåõîä èç="'||:v_now_status_sign||'" â="'||:i_new_status_sign||'")';
...
begin
  ...
  begin
    select pt.placetype_code from placetypes pt
      ...
      into :v_placetype_code;
    if (:v_placetype_code is null) then
      exception ex_unknown_placetype '=õø÷òõ¸ªývù ªøÿ ýð¸õûõýýþóþ ÿºýúªð "'||v_value||'"';
  end
  ...
  begin
    ...
    if (v_value is null) then
      exception ex_areaname_expected '+ª¸ºª¸ªòºõª ýð÷òðýøõ ¨ðùþýð';
