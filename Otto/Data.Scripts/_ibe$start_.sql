/******************************************************************************/
/***          Generated by IBExpert 2010.03.23 23.11.2011 17:19:37          ***/
/******************************************************************************/
...
DECLARE EXTERNAL FUNCTION ESCAPESTRINGEX
    CSTRING(4000) CHARACTER SET CYRL,
    CSTRING(50) CHARACTER SET CYRL
    RETURNS CSTRING(4000) CHARACTER SET CYRL FREE_IT
...
VARCHAR(4000) CHARACTER SET CYRL;
...
BLOB SUB_TYPE 1 SEGMENT SIZE 80 CHARACTER SET CYRL;
...
CREATE OR ALTER PROCEDURE SPLITSTRING (
    I_STRING TYPE OF NODE_VALUE,
    I_DEVIDER TYPE OF DEVIDER)
RETURNS (
    O_HEAD TYPE OF NODE_VALUE,
    O_TILE TYPE OF NODE_VALUE)
AS
BEGIN
  SUSPEND;
END^
...
CREATE TABLE ACCOPERS (
    ...
    BALANCE_EUR  MONEY_EUR NOT NULL
);
...
CREATE TABLE DEALACTIONS (
    DEAL_ID    ID_DEAL NOT NULL,
    ACTION_ID  ID_ACTION NOT NULL
);
...
CREATE TABLE IMP_CITYGET (
    CITY_ID    VARCHAR(7) CHARACTER SET CYRL,
    CITY_RUS   VARCHAR(20) CHARACTER SET CYRL,
    CITY_ENG   VARCHAR(20) CHARACTER SET CYRL,
    INDEXCITY  VARCHAR(6) CHARACTER SET CYRL,
    KODTEL     VARCHAR(10) CHARACTER SET CYRL,
    DATE_TIME  VARCHAR(255) CHARACTER SET CYRL,
    USERNAME   VARCHAR(25) CHARACTER SET CYRL,
    REGION     VARCHAR(40) CHARACTER SET CYRL,
    REG_ENG    VARCHAR(40) CHARACTER SET CYRL,
    CITYREG    VARCHAR(63) CHARACTER SET CYRL,
    TYPE_ID    VARCHAR(7) CHARACTER SET CYRL,
    REGION_ID  VARCHAR(7) CHARACTER SET CYRL,
    OBLAST_ID  VARCHAR(7) CHARACTER SET CYRL
);

CREATE TABLE IMP_CITYTYPE (
    TYPE_ID    VARCHAR(7) CHARACTER SET CYRL,
    NAMETYPE   VARCHAR(5) CHARACTER SET CYRL,
    NAMEENG    VARCHAR(5) CHARACTER SET CYRL,
    REMARK     VARCHAR(25) CHARACTER SET CYRL,
    DATE_TIME  VARCHAR(255) CHARACTER SET CYRL,
    USERNAME   VARCHAR(25) CHARACTER SET CYRL
);

CREATE TABLE IMP_CLIENT (
    CLIENT_ID   VARCHAR(7) CHARACTER SET CYRL,
    FAMILY      VARCHAR(40) CHARACTER SET CYRL,
    FAMILYENG   VARCHAR(25) CHARACTER SET CYRL,
    STREET      VARCHAR(30) CHARACTER SET CYRL,
    STREETENG   VARCHAR(30) CHARACTER SET CYRL,
    HOME        VARCHAR(15) CHARACTER SET CYRL,
    CITYGET_ID  VARCHAR(7) CHARACTER SET CYRL,
    TELEPHONE   VARCHAR(40) CHARACTER SET CYRL,
    DATE_TIME   VARCHAR(19) CHARACTER SET CYRL,
    CH_USER     VARCHAR(25) CHARACTER SET CYRL,
    MOBIL       VARCHAR(7) CHARACTER SET CYRL,
    EMAIL       VARCHAR(30) CHARACTER SET CYRL,
    GOS_ID      VARCHAR(6) CHARACTER SET CYRL,
    KODMOB      VARCHAR(5) CHARACTER SET CYRL,
    BLACKLIST   INTEGER,
    BLACKREM    VARCHAR(50) CHARACTER SET CYRL,
    PUBLICITY   VARCHAR(7) CHARACTER SET CYRL,
    CH_DATE     VARCHAR(255) CHARACTER SET CYRL,
    PASPORT     VARCHAR(10) CHARACTER SET CYRL,
    PUT         VARCHAR(50) CHARACTER SET CYRL,
    FFAKSNEW    INTEGER,
    FAKSNEW     VARCHAR(12) CHARACTER SET CYRL,
    FADDRESS    INTEGER,
    BANKRS      VARCHAR(20) CHARACTER SET CYRL,
    BANKNS      VARCHAR(20) CHARACTER SET CYRL,
    BANKNAME    VARCHAR(200) CHARACTER SET CYRL,
    BANKADRES   VARCHAR(200) CHARACTER SET CYRL,
    BANKKOD     VARCHAR(20) CHARACTER SET CYRL,
    BANKEASY    VARCHAR(20) CHARACTER SET CYRL,
    BANKUNP     VARCHAR(20) CHARACTER SET CYRL,
    PERSONNUM   VARCHAR(14) CHARACTER SET CYRL,
    PUTDATE     VARCHAR(255) CHARACTER SET CYRL,
    PRIZMY      INTEGER
);

CREATE TABLE IMP_OBLAST (
    OBLAST_ID  VARCHAR(7) CHARACTER SET CYRL,
    NAMEOBL    VARCHAR(20) CHARACTER SET CYRL,
    NAMEENG    VARCHAR(20) CHARACTER SET CYRL,
    DATE_TIME  VARCHAR(255) CHARACTER SET CYRL,
    USERNAME   VARCHAR(25) CHARACTER SET CYRL
);

CREATE TABLE IMP_REGION (
    REGION_ID  VARCHAR(7) CHARACTER SET CYRL,
    NAMEREG    VARCHAR(20) CHARACTER SET CYRL,
    NAMEENG    VARCHAR(20) CHARACTER SET CYRL,
    OBLAST_ID  VARCHAR(7) CHARACTER SET CYRL,
    DATE_TIME  VARCHAR(255) CHARACTER SET CYRL,
    USERNAME   VARCHAR(25) CHARACTER SET CYRL
...
CREATE TABLE INVOICE_ATTRS (
    OBJECT_ID   ID_INVOICE NOT NULL,
    ATTR_ID     ID_ATTR NOT NULL,
    ATTR_VALUE  VALUE_ATTR
);

CREATE TABLE INVOICES (
    INVOICE_ID    ID_INVOICE NOT NULL,
    INVOICE_CODE  CODE_ORDER,
    CREATE_DTM    DTM_CREATE,
    ORDER_ID      ID_ORDER NOT NULL,
    AMOUNT_EUR    MONEY_EUR,
    BYR2EUR       VALUE_INTEGER NOT NULL,
...
CREATE TABLE PHONEPREFIXES (
    PHONEPREFIX_CODE  CODE_PHONEPREFIX NOT NULL,
    PHONETYPE_CODE    CODE_PHONETYPE NOT NULL
);

CREATE TABLE PHONETYPES (
    PHONETYPE_CODE  CODE_PHONETYPE NOT NULL,
    PHONETYPE_NAME  NAME_REF NOT NULL
);
...
CREATE TABLE PLACES (
    ...
    STATUS_ID         ID_STATUS NOT NULL,
    PHONEPREFIX_CODE  CODE_PHONEPREFIX
);
...
/* View: V_ACCOPER_SUMMARY */
CREATE OR ALTER VIEW V_ACCOPER_SUMMARY(
    ACCOUNT_ID,
    DEAL_ID,
    ACTION_SIGN,
    AMOUNT_EUR,
    ACCOPER_ID)
AS
SELECT ao.account_id, da.deal_id, a.action_sign, sum(ao.amount_eur) amount_eur, max(ao.accoper_id) accoper_id
from accopers ao
  inner join actions a on (a.action_id = ao.action_id)
  inner join dealactions da on (da.action_id = ao.action_id)
  inner join clients cl on (cl.account_id = ao.account_id)
group by ao.account_id, da.deal_id, a.action_sign
;
...
select ph.param_id, ph.object_id, a.attr_id, p.param_value
  from paramheads ph
    ...
    inner join attrs a on (a.attr_sign = p.param_name and a.object_sign = ph.object_sign and a.field_name is null)
;
...
select
  c.client_id,
  cast(c.last_name||' '||c.first_name||' '||c.mid_name as name_ref)
from clients c
;



/* View: V_INVOICE_ATTRS */
CREATE OR ALTER VIEW V_INVOICE_ATTRS(
    OBJECT_ID,
    ATTR_SIGN,
    ATTR_VALUE,
    ATTR_NAME)
