...
procedure ExportSuissen(aTransaction: TpFIBTransaction; ndProduct: TGvXmlNode;
  aProductId: integer; aStatusSign, aArjName:String);

implementation

uses
  SysUtils, Dialogs, dbf, udmOtto, GvXmlUtils, GvStr, GvFile;

procedure ExportOrder(aTransaction: TpFIBTransaction; ndProduct, ndOrders: TGvXmlNode;
  aOrderId: integer; aStatusSign:String; dbfClient, dbfOrder, dbfItems: TDbf);
var
  ndOrder, ndClient, ndAdress, ndPlace, ndOrderItems, ndOrderItem: TGvXmlNode;
...
begin
  ...
  if not ndClient.HasAttribute('SUISSEN_CODE') then
  begin
    ndClient['SUISSEN_CODE']:= dmOtto.GetNextCounterValue('CLIENT', 'SUISSEN_CODE', 0, aTransaction);
    dmOtto.ActionExecute(aTransaction, ndClient);
  end;
  ndAdress:= ndClient.AddChild('ADRESS');
  dmOtto.AdressRead(ndAdress, ndOrder['ADRESS_ID'], aTransaction);
  ndPlace:= ndAdress.Find('PLACE');
  dbfClient.Append;
  BatchMoveFields(dbfClient, ndClient,
    ...
    'PHONE=MOBILE_PHONE');
  BatchMoveFields(dbfClient, ndAdress,
    'ADRESS_NAT=STREET_TEXT;ZIP=POSTINDEX');
  BatchMoveFields(dbfClient, ndPlace,
    'CITY_NAT=PLACE_TEXT');
  ...
  dbfOrder.Append;
  BatchMoveFields(dbfOrder, ndClient,
    'CLIENT_NUM=SUISSEN_CODE;');
  BatchMoveFields(dbfOrder, ndOrder,
  ...
  begin
    ...
    if not ndOrderItem.HasAttribute('AUFTRAG_ID') then
    begin
      ...
      ndOrderItem['AUFTRAG_ID']:= Intern;
    end;
    dbfItems.Append;
    BatchMoveFields(dbfItems, ndClient,
      'CLIENT_NUM=SUISSEN_CODE');
    BatchMoveFields(dbfItems, ndOrder,
      'ORDER_NUM=ID;ORDER_DATE=CREATE_DTM');
    BatchMoveFields(dbfItems, ndOrderItem,
