...
ALTER TABLE MONEYBACKS DROP CONSTRAINT FK_MONEYBACKS_ACCOUNT;

/* Create Foreign Key... */
RECONNECT;

ALTER TABLE MONEYBACKS ADD CONSTRAINT FK_MONEYBACKS_ACCOUNT FOREIGN KEY (ACCOUNT_ID) REFERENCES ACCOUNTS (ACCOUNT_ID) ON UPDATE CASCADE ON DELETE CASCADE;

/* Alter Procedure... */
/* empty dependent procedure body */
/* Clear: ACTION_RUN for: ACT_ACCOUNT_CREDIT */
SET TERM ^ ;
...
begin
  ...
  begin
    if (abs(v_amount_eur) < abs(v_acc_amount_eur)) then
    ...
    execute procedure param_set(:i_param_id, 'AMOUNT_BYR', :v_delta_byr);
    execute procedure param_set(:i_param_id, 'AMOUNT_EUR', :v_delta_eur);
    select o_value from param_get(:i_param_id, 'NOTES') into :v_notes;
    select o_pattern from param_fillpattern(:i_param_id, :v_notes) into :v_notes;
...
begin
  ...
  begin
    ...
    begin
      ...
      v_delta_eur = (1.00 * v_delta_byr) / v_byr2eur;
      v_amount_eur = v_amount_eur - v_delta_eur;
      update accrests ar
