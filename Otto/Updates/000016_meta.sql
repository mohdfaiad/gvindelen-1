...
ALTER TABLE ACCOPERS ADD ORDER_ID ID_ORDER;

ALTER TABLE ACCOPERS ADD ORDERITEM_ID ID_ORDERITEM;

ALTER TABLE ACCOPERS ADD ORDERTAX_ID ID_TAX;

ALTER TABLE ACCOPERS ADD NOTES VALUE_ATTR;

ALTER TABLE ORDERITEMS ADD PAID_EUR MONEY_EUR;

ALTER TABLE ORDERTAXS ADD PAID_EUR MONEY_EUR;

/* Alter Field (Null / Not Null)... */
UPDATE RDB$RELATION_FIELDS SET RDB$NULL_FLAG = NULL WHERE RDB$FIELD_NAME='ACTION_ID' AND RDB$RELATION_NAME='ACCOPERS';


/* Create Procedure... */
SET TERM ^ ;

CREATE PROCEDURE ACT_ORDERITEM_PAY(I_PARAM_ID TYPE OF ID_PARAM,
...
CREATE PROCEDURE TAX_USE_REST(I_TAXRATE_ID TYPE OF ID_TAX,
...
/* Drop altered view: V_ATTRINPARAM */
SET TERM ; ^

DROP VIEW V_ATTRINPARAM;

/* Create altered view: V_ATTRINPARAM */
/* Create view: V_ATTRINPARAM (ViwData.CreateDependDef) */
CREATE VIEW V_ATTRINPARAM(
PARAM_ID,
OBJECT_ID,
ATTR_ID,
ATTR_VALUE)
...
begin
  ...
  update accopers ao
    set ao.order_id = null, ao.orderitem_id = null,  ao.ordertax_id = null;
...
/* Clear: ACTION_RUN for: ACT_ORDERITEM_STORE */
...
/* Restore proc. body: ACT_ORDERITEM_PAY */
...
begin
  ...
  select oi.cost_eur - oi.paid_eur
    from orderitems oi
    where oi.orderitem_id = :i_object_id
    into v_amount_eur;
...
/* Restore proc. body: TAX_USE_REST */
ALTER PROCEDURE TAX_USE_REST(I_TAXRATE_ID TYPE OF ID_TAX,
I_PARAM_ID TYPE OF ID_PARAM)
 RETURNS(O_COST_EUR TYPE OF MONEY_EUR)
 AS
...
GRANT ALL ON V_ATTRINPARAM TO SYSDBA WITH GRANT OPTION;
