...
/*  Empty ACT_ACCOUNT_CREDITORDER for PARAM_FILL(param list change)  */
SET TERM ^ ;

ALTER PROCEDURE ACT_ACCOUNT_CREDITORDER(I_PARAM_ID /* TYPE OF ID_PARAM */ BIGINT NOT NULL,
...
/*  Empty ACT_MONEYBACK_CREATE for PARAM_FILL(param list change)  */
ALTER PROCEDURE ACT_MONEYBACK_CREATE(I_PARAM_ID /* TYPE OF ID_PARAM */ BIGINT,
I_OBJECT_ID /* TYPE OF ID_OBJECT */ INTEGER,
I_OBJECT_SIGN /* TYPE OF SIGN_OBJECT */ VARCHAR(30) = 'MONEYBACK')
 AS
 BEGIN EXIT; END
^

/* Alter empty procedure PARAM_FILL with new param-list */
...
/* Alter (ACT_MONEYBACK_CREATE) */
ALTER PROCEDURE ACT_MONEYBACK_CREATE(I_PARAM_ID TYPE OF ID_PARAM,
...
begin
  ...
  insert into moneybacks(moneyback_id, account_id, amount_byr, kind)
    values(:i_object_id, :v_account_id, :v_amount_byr, :v_moneyback_kind);
...
/* Altering existing trigger... */
ALTER TRIGGER ORDERITEMS_AIUD0
AS
begin
...
/* Altering existing trigger... */
ALTER TRIGGER ORDERMONEYS_AIUD0
AS
begin
  execute procedure order_updatecost(coalesce(new.order_id, old.order_id));
end
